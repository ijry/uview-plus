<template>
	<view class="u-picker-warrper">
		...
		<u-popup :show="show || (hasInput && showByClickInput)" :mode="popupMode" :zIndex="zIndex"
			@close="closeHandler">
			<view class="u-picker">
				<u-toolbar v-if="showToolbar" :cancelColor="cancelColor" :confirmColor="confirmColor"
					:cancelText="cancelText" :confirmText="confirmText" :title="title"
					:rightSlot="toolbarRightSlot ? true : false" @cancel="cancel" @confirm="confirm">
					<template #right>
						<slot name="toolbar-right"></slot>
					</template>
				</u-toolbar>
				<slot name="toolbar-bottom"></slot>
				<picker-view class="u-picker__view" :indicatorStyle="`height: ${addUnit(itemHeight)}`"
					...
					<picker-view-column v-for="(item, index) in innerColumns" :key="index"
						...
						<view v-if="testArray(item)" class="u-picker__view__column__item u-line-1"
	...
	export default {
		...
		methods: {
			...
			confirm() {
				...
				//带输入框的picker，change时候不会修改v-model数据，所以确认时候需要更新外面数据
				if (this.hasInput) {
