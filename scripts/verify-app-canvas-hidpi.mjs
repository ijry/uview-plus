...
const initialization = canvas.slice(
  canvas.indexOf('async _initializeCanvas'),
  canvas.indexOf('this.applyFont()')
)
...
assert.match(canvas, /\/\/ #ifdef MP \|\| H5 \|\| APP-PLUS[\s\S]*?destWidth = Math\.round\(width \* this\.dpr\)/)
assert.match(canvas, /\/\/ #ifdef MP \|\| H5 \|\| APP-PLUS[\s\S]*?destHeight = Math\.round\(height \* this\.dpr\)/)
assert.match(initialization, /\/\/ #ifdef MP[\s\S]*?this\._canvasElement\.width = Math\.ceil\(this\.actualWidth \* this\.dpr\)/)
assert.match(initialization, /\/\/ #ifdef MP[\s\S]*?this\.ctx\.setTransform\(this\.dpr, 0, 0, this\.dpr, 0, 0\)/)
assert.doesNotMatch(initialization, /\/\/ #ifdef MP \|\| H5[\s\S]*?this\._canvasElement\.width = Math\.ceil\(this\.actualWidth \* this\.dpr\)/)
...
console.log('MP/H5/APP-PLUS canvas HiDPI export assertions passed')
